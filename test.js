...
export default class AgendaView extends Component {
  ...
  generateMarkings() {
    ...
    const key = this.state.selectedDay.toString('dd-MM-yyyy');
  ...
  render() {
    ...
    const scrollPadScrollStyle = [{ translateY: this.state.scrollY.interpolate({
      inputRange: [0, agendaHeight],
      outputRange: [agendaHeight, 0],
      extrapolate: 'clamp',
    })}]
    ...
    const scrollPadStyle = {
      ...
      width: 80,
      height: KNOB_HEIGHT,
      top: HEADER_HEIGHT- KNOB_HEIGHT,
      left: (this.viewWidth - 80) / 2,
      transform: scrollPadScrollStyle,
    };

    let knob = (<View style={this.styles.knobContainer}/>);

      const knobView = this.state.knobStyle=='up' ? <TabBarIcon 
        name = "ios-arrow-up"
        brand="Ionicons"
        focus={true}
